...
public class DataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        seedFoodItems(allCategories);
        List<FoodItem> allFoodItems = foodItemRepository.findAll();
        seedMenus(allFoodItems);
        List<Menu> allMenus = menuRepository.findAll();
        seedTables();
        List<TableTop> allTables = tableTopRepository.findAll();
        seedUsers(allAddresses);
        List<User> allUsers = userRepository.findAll();
        seedOrders(allUsers, allAddresses, allTables);
        List<Order> allOrders = orderRepository.findAll();
        seedOrderItems(allFoodItems, allOrders);
        seedCallRequests();
    ...
    private void seedCategories() throws IOException {
        if (categoryRepository.count() == 0) {
            ...
            List<Category> categories = readJsonList("seed-categories.json", new TypeReference<List<Category>>() {});
            categoryRepository.saveAll(categories);
        }
    }

    private void seedFoodItems(List<Category> allCategories) throws IOException {
    ...
    private void seedMenus(List<FoodItem> allFoodItems) throws IOException {
            ...
            for (Map<String, Object> raw : menusRaw) {
                ...
                if (raw.get("foodItems") instanceof List) {
                    for (String foodName : (List<String>) raw.get("foodItems")) {
                        allFoodItems.stream().filter(f -> f.getFoodName().equals(foodName)).findFirst().ifPresent(menuItems::add);
                    }
                }
    ...
    private void seedUsers(List<Address> allAddresses) throws IOException, ParseException {
        if (userRepository.count() == 0) {
            ...
            for (Map<String, Object> raw : usersRaw) {
                ...
                if (raw.get("lastLogin") != null) u.setLastLogin(LocalDateTime.parse((String) raw.get("lastLogin"), dateTimeFormat));
                ...
                if (raw.get("addresses") instanceof List) {
                    for (String addrName : (List<String>) raw.get("addresses")) {
                        allAddresses.stream().filter(a -> a.getName().equals(addrName)).findFirst().ifPresent(userAddresses::add);
                    }
                }
    ...
    private void seedOrders(List<User> allUsers, List<Address> allAddresses, List<TableTop> allTables) throws IOException {
        if (orderRepository.count() == 0) {
            ...
            for (Map<String, Object> raw : ordersRaw) {
                ...
                if (raw.get("customer") != null) {
                    allUsers.stream().filter(u -> u.getEmail().equals(raw.get("customer"))).findFirst().ifPresent(o::setCustomer);
                }
                if (raw.get("address") != null) {
                    allAddresses.stream().filter(a -> a.getName().equals(raw.get("address"))).findFirst().ifPresent(o::setAddress);
                }
                if (raw.get("table") != null) {
                    Integer tableNum = (Integer) raw.get("table");
                    allTables.stream().filter(t -> t.getTableNumber().equals(tableNum)).findFirst().ifPresent(o::setTable);
                }
                orders.add(o);
            }
    ...
    private void seedOrderItems(List<FoodItem> allFoodItems, List<Order> allOrders) throws IOException {
        if (orderItemRepository.count() == 0) {
            ...
            for (Map<String, Object> raw : orderItemsRaw) {
                ...
                if (raw.get("foodItem") != null) {
                    allFoodItems.stream().filter(f -> f.getFoodName().equals(raw.get("foodItem"))).findFirst().ifPresent(oi::setFoodItem);
                }
    ...
    private void seedCallRequests() throws IOException {
        if (callRequestRepository.count() == 0) {
            ...
            for (Map<String, Object> raw : callsRaw) {
                ...
                cr.setType((String) raw.get("type"));
                cr.setStatus((String) raw.get("status"));
